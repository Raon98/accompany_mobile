...
import { modalContentProps } from './Terms'
...
const ASU0101P01 = () => {
  const { signState, onState, onReset, onBox, openBox, onSuccessForm, setSucces } = useSign()
        ...
        Props1: {
          ...
          confirmFn: () => {
            const sign = {
              ...
              birth: signData.birthYy.value + signData.birthMm.value + signData.birthDd.value,
  ...
  const func = {
    ...
    onValidation: (currentName: keyof SignState): boolean => {
      ...
      if (currentName === 'password') {
        ...
        if (!regex.test(signData.password.value)) {
          validText.current = '8자이상, 숫자, 특수문자가 포함됐는지 확인해주세요.'
          onState(currentName, 'fail')
          return false
        }
      }
    ...
    onNext: (
      ...
      if (length && target.value.length >= length && !allowedKeys.includes(e.key)) {
        e.preventDefault()
      }
    ...
    onSubmit: (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (true) {
        setModalContent({
          Props1: {
            content: '가입을 완료하시겠습니까?<br/>소중한 순간을 동행하다와 함께하세요.',
            cancel: true,
            confirmText: '확인',
            cancelText: '취소',
            confirmFn: () => {
  ...
  return (
    <>
      ...
      <form className="sign" onSubmit={func.onSubmit}>
        {!onSuccessForm && <div className="sign__title">{title.current} 입력해주세요.</div>}
        <div className="sign-content">
          <div className={`sign__block gender ${signState('gender', 'state') ? 'show' : 'hide'}`}>
            <label htmlFor="gender">성별 </label>
            <div className="gender__block">
              <button
                type="button"
                className={`bdr__btn ${signData.gender.value === 'm' ? 'primary' : ''} `}
              ...
              <button
                type="button"
                className={`bdr__btn ${signData.gender.value === 'g' ? 'primary' : ''} `}
          ...
          <div className={`sign__block birth ${signState('birthYy', 'state') ? 'show' : 'hide'}`}>
          ...
          <div className={`sign__block phone ${signState('phone', 'state') ? 'show' : 'hide'}`}>
            ...
            <input
              ...
              min={0}
            />
            {signState('phone', 'fail') && <div className="info__text">{validText.current}</div>}
          </div>
          <div className={`sign__block email ${signState('email', 'state') ? 'show' : 'hide'}`}>
              ...
              <div className="emailAddress">
                ...
                <ul className={`emailAddress__option--select ${onBox ? 'show' : ''}`}>
                  {optionList.map((domain, idx) => (
                    <li key={idx}>
                      <button type="button" className="option-btn" onClick={() => func.selOption(domain)}>
                        {domain}
                      </button>
                    </li>
                  ))}
          ...
          <div className={`sign__block name ${signState('name', 'state') ? 'show' : 'hide'}`}>
          ...
          >
            ...
            />
            {signState('password', 'fail') && <div className="info__text">{validText.current}</div>}
            ...
            />
            {signState('passwordConfirm', 'fail') && <div className="info__text">{validText.current}</div>}
          </div>
          <div className="sign__block uid">
            ...
            <div className="uid__form">
              <input
                ...
                required
              />
              <button className="uid-check__btn" type="button" onClick={() => func.onCheck()}>
                중복확인
              </button>
            </div>
