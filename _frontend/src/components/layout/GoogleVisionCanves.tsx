...
const GoogleVisionCanves = () => {
  ...
  const visionMutation = useMutation({
    mutationFn: (imageData: string) => $api('ext', 'GoogleVision', 'DrawingCanvass', { base64Image: imageData }),
    onSuccess: res => {
  ...
  return (
    <div>
      ...
      <button onClick={() => (lang === Language.KOREAN ? setLang(Language.ENGLISH) : setLang(Language.KOREAN))}>
        언어변경
      </button>
